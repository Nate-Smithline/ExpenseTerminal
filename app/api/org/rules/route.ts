...
import { orgRulePostSchema } from "@/lib/org-rules/schemas";
...
export async function GET(req: Request) {
  ...
  if (error) {
    return NextResponse.json(
      { error: safeErrorMessage(error.message, "Failed to load rules") },
      { status: 500 },
    );
  }
...
export async function POST(req: Request) {
  ...
  const parsed = orgRulePostSchema.safeParse(body);
  if (!parsed.success) {
    const msg = parsed.error.flatten().formErrors[0] ?? "Invalid rule";
    return NextResponse.json({ error: msg }, { status: 400 });
  }

  const { name, enabled, position, conditions, actions, trigger_mode } = parsed.data;
  ...
  const row = {
    ...
    trigger_mode: trigger_mode ?? "new_and_existing",
